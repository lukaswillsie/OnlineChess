...
public class DrawHelper extends SubHelper implements ReturnCodeCaller {
    ...
    public void onServerReturn(int code) {
        switch (code) {
            ...
            case ReturnCodes.Draw.SUCCESS:
                Log.e(tag, "Server says draw offer/acceptance was a success");
    ...
    /**
     * We use this method so that we can give callbacks to the UI thread on the UI thread (as
     * opposed to our worker thread), so as to avoid exceptions from editing the UI thread off of
     * the UI thread.
     *
