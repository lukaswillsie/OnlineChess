...
public enum ServerData {
    ...
    STATE('i', 3),				// State of the game; 0 if white's turn, 1 if black's turn
    TURN('i', 4),				// Current turn number
    WHITE_ARCHIVED('i', 5),	    // 1 if the white user has archived this game, 0 otherwise
    BLACK_ARCHIVED('i', 6),	    // 1 if the black user has archived this game, 0 otherwise
    DRAW_OFFERED('i', 7),		// 1 if a draw has been offered, and the player whose turn it is
                                    // needs to respond
    DRAWN('i', 8),				// 1 if the players in this game have agreed to a draw, 0 otherwise
    WINNER('s', 9),				// Contains the name of the winner, or the empty String if there isn't a winner
    WHITE_CHK('i', 10),		    // 1 if white is in check, 0 if not
    BLACK_CHK('i', 11),		    // 1 if black is in check, 0 if not
    PROMOTION_NEEDED('i', 12);	// 1 if the player whose turn it is needs to promote a piece before their turn can end
    ...
    // it explicit here.
    public static final ServerData[] order = {GAMEID, WHITE, BLACK, STATE, TURN, WHITE_ARCHIVED, BLACK_ARCHIVED, DRAW_OFFERED, DRAWN, WINNER, WHITE_CHK, BLACK_CHK, PROMOTION_NEEDED};
